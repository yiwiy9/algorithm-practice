...
/**
 * 頂点s,t間の最短経路として考えられるものはいろいろ考えられるが、そのいずれの経路にも含まれないような辺については、伸ばしても特に意味がない
 * => １つ以上ある最短経路が通る辺のみでグラフを構成する（ワーシャル・フロイド）
 * => あとは最小カット問題（フォード・ファルカーソン）
 */
int main() {
  int N, M, s, t;
  cin >> N >> M >> s >> t;
  ...
  for (int u=0; u<N; u++) {
    for (int v=0; v<N; v++) {
      ...
      if (dp[s][u] + dp[u][v] + dp[v][t] != dp[s][t]) continue;
