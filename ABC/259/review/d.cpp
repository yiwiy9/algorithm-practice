...
bool is_cross(long long dist, long long r1, long long r2)
{
...
int main()
{
  ...
  for (int i=0; i<N; i++) {
    for (int j=0; j<N; j++) {
      long long dist = distance(x[i], y[i], x[j], y[j]);
      if (is_cross(dist, r[i], r[j])) {
        G[i].push_back(j);
      }
    }
    if (distance(x[i], y[i], sx, sy) == r[i]*r[i]) {
      start.push_back(i);
    }
    if (distance(x[i], y[i], tx, ty) == r[i]*r[i]) {
      end.push_back(i);
    }
  }
