...
bool is_square(int a, int b, int c, int d)
{
  ...
  if (ab == ac) {
    return ab == dist(X[b],Y[b],X[d],Y[d]) && ab == dist(X[c],Y[c],X[d],Y[d]);
  }
  if (ab == ad) {
    return ab == dist(X[b],Y[b],X[c],Y[c]) && ab == dist(X[c],Y[c],X[d],Y[d]);
  }
  if (ad == ac) {
    return ad == dist(X[b],Y[b],X[d],Y[d]) && ad == dist(X[c],Y[c],X[b],Y[b]);
  }
  return false;
}
